...
class ISPSurface:
    ...
    def __str__(self):
        s = "surface: [name: %s index: %d phys: 0x%x iova: 0x%x size: 0x%x]" % (
            self.name,
            self.index,
            self.phys,
            self.iova,
...
class ISP:
    def __init__(self, u):
        ...
        self.dart = DART.from_adt(
            self.u, "/arm-io/dart-isp", iova_range=(0x3A28000, 0x20000000)
        )  # +0x4000 extra heap
    ...
    def write_fabric_tunables(self):
        ...
        p.mask32(
            self.isp_base + 0x738, 0x1FF01FF, 0x2
        )  # these 4 are used in power reset
    ...
    def write_dapf_tunables(self):
        ...
        while offset < len(dapf_cfg):
            (start, end, _, r0h, r0l, _, r4) = struct.unpack_from(
                "QQBBBBI", buffer=dapf_cfg, offset=offset
            )
            offset += 24
            p.write32(dapf_base + 0x4, r4)
            p.write32(dapf_base + 0x8, start & 0xFFFFFFFF)
            p.write32(
                dapf_base + 0xC, start >> 32
            )  # macos does 32 bit writes, can we do 64?
    ...
    def initialize_firmware(self):
        ...
        (text_phys, text_iova, _, text_size, data_phys, data_iova, _, data_size) = (
            struct.unpack(
                "<QQQI4xQQQI4x", getattr(self.u.adt["/arm-io/isp"], "segment-ranges")
            )
        )
        ...
        bootargs_iova = (
            ipc_iova + ipc_args_offset
        ) + 0x40  # (0x1824000 + 0xef40) + 0x40; 0x1812f80
        cmd_iova = (
            bootargs_iova + ISPIPCBootArgs.sizeof()
        ) + 0x40  # (0x1812f80 + 0x180) + 0x40; 0x1813140
        self.cmd_iova = cmd_iova

        bootargs = ISPIPCBootArgs.build(
            dict(
                ipc_iova=ipc_iova,
                unk0=0x1800000,
                unk1=0xE800000,

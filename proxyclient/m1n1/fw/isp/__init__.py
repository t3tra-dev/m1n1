...
class ISPChannel:
    ...
    def log(self, *args):
        if not self._stfu:
            if args:
                print("ISP: %s:" % (self.abbrev.upper()), *args)
            else:
                print()
    ...
    def read_msg(self, index):
        assert index < self.num
        return ISPChannelMessage.parse(
            self.isp.ioread(self.get_iova(index), self.entry_size)
        )
    ...
    def read_all_msgs(self):
        ...
        return [
            ISPChannelMessage.parse(
                ring[n * self.entry_size : (n + 1) * self.entry_size]
            )
    ...
    def send(self, req):
        ...
        if rsp == None:
            raise RuntimeError("ASC never acked. IRQ stuck.")
        return rsp

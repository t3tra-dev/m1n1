...
class ISPBufT2HChannel(ISPChannel):
    ...
    def _handle(self, req):
        assert req.arg1 > 0x0
        self.log(
            "RX: REQ: [iova: 0x%x, size: 0x%x, flag: 0x%x]"
        ...
        self.log(
            "TX: RSP: [iova: 0x%x, ack: 0x%x, flag: 0x%x]"
            % (rsp.arg0, rsp.arg1, rsp.arg2)
        )
        return rsp
...
class ISPSharedMallocChannel(ISPChannel):
    ...
    def _handle(self, req):
        ...
        if req.arg0 == 0:
            ...
            self.log(
                "SM: TX: Malloc RSP: [iova: 0x%x, index: 0x%x]" % (rsp.arg0, rsp.arg2)
            )
...
class ISPChannelTable:
    def __init__(self, isp, description):
        ...
        for desc in description:
            name = desc.name
            if name == "TERMINAL":
                self.terminal = ISPTerminalChannel(isp, desc, abbrev="TM")
            if name == "IO":
                self.io = ISPIOChannel(isp, desc, abbrev="IO")
            if name == "DEBUG":
                self.debug = ISPDebugChannel(isp, desc, abbrev="DG")
            if name == "BUF_H2T":
                self.bufh2t = ISPBufH2TChannel(isp, desc, abbrev="BR")
            if name == "BUF_T2H":
                self.buft2h = ISPBufT2HChannel(isp, desc, abbrev="BT")
            if name == "SHAREDMALLOC":
                self.sharedmalloc = ISPSharedMallocChannel(isp, desc, abbrev="SM")
            if name == "IO_T2H":
                self.iot2h = ISPIOT2HChannel(isp, desc, abbrev="IT")
        self.channels = [
            self.terminal,
            self.io,
            self.debug,

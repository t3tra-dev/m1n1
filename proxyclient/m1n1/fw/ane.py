...
from m1n1.hw.ane import ANEDARTRegs, ANERegs, ANETaskManager
...
class ANE:
    ...
    def __init__(self, u):
        ...
        self.dart = DART.from_adt(
            u,
            path=f"/arm-io/dart-{self.name}",
            instance=0,
            iova_range=(0x4000, 0xE0000000),
        )
...
class ANEAllocator:
    ...
    def alloc_size(self, size):
        ...
        print(
            "mapid %d: mapped phys 0x%x to iova 0x%06x for data w/ size 0x%06x"
    ...
    def dump_map(self):
        for mapid in self.map:
            buf = self.map[mapid]
            print(
                "mapid %d: phys 0x%x, iova 0x%x, size 0x%x"
                % (buf.mapid, buf.phys, buf.iova, buf.size)
            )

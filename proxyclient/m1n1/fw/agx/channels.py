...
class StatsMsg_Temp(ConstructClass):
    ...
    def __str__(self):
        ...
        return (
            f"Temp: {temp:.2f}°C s={self.scale:d} tmin={self.tmin:d} tmax={self.tmax:d}"
        )
    ...
    def __str__(self):
        ...
        if code in TRACE_MSGS:
            ...
            args = (
                ...
                + ": "
                + " ".join(
                    f"{k}={v:#x}" for k, v in zip(info[1:], self.args) if k is not None
                )
            )
        else:
...
channelRings = [[(RunCmdQueueMsg, RunCmdQueueSize, 0x100)]] * 12 + [
...
class Channel(Reloadable):
    def __init__(
        self, u, uat, info, ring_defs, base=None, state_fields=ChannelStateFields
    ):
        self.uat = uat
        self.u = u
        self.p = u.proxy

...
from construct.core import Int32sl, Int32ul, Int64ul
...
class BufferManagerBlockControl(ConstructClass):
    subcon = Struct(
        "total" / Int32ul, "wptr" / Int32ul, "unk" / Int32ul, "pad" / ZPadding(0x34)
    )


class BufferManagerCounter(ConstructClass):
    subcon = Struct("count" / Int32ul, "pad" / ZPadding(0x3C))
...
class Start3DStruct7(ConstructClass):
    subcon = Struct(
        ...
        "stamp2_addr"
        / WrappedPointer,  # same as FinalizeComputeCmd.stamp - some kind of fence/token
...
class Start3DCmd(ConstructClass):
    subcon = Struct(  # 0x194 bytes''''
        ...
        "cmdqueue_ptr"
        / Int64ul,  # points back to the CommandQueueInfo that this command came from
        "workitem_ptr"
        / Int64ul,  # points back at the WorkItem that this command came from
        "context_id" / Int32ul,
        "unk_50" / Int32ul,
...
class StartTACmdStruct2(ConstructClass):
    subcon = Struct(
        ...
        "tpc" / Hex(Int64ul),
        "tvb_heapmeta_addr"
        / Hex(Int64ul),  # like Start3DStruct2.tvb_end_addr with bit 63 set?
...
class ComputeInfo(ConstructClass):
    ...
    subcon = Struct(  # 0x1c bytes
        ...
        "pipeline_base"
        / Int64ul,  # 0x11_00000000: Used for certain "short" pointers like pipelines (and shaders?)
...
class FinalizeComputeCmd(ConstructClass):
    subcon = Struct(  # 0x64 bytes''''
        ...
        "restart_branch_offset"
        / Int32sl,  # relative offset from start of Finalize to StartComputeCmd
        "unk_60" / Int32ul,
...
class MicroSequence(ConstructValueClass):
    subcon = RepeatUntil(
        lambda obj, lst, ctx: lst[-1].op & 0x3F in (0x18, 0x2B, 0x2C),
        Struct(
            "op" / Peek(Int32ul),
            # Probe(lookahead=32),
            "cmd"

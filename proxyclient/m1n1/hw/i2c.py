...
class I2CRegMapDev:
    ...
    def _switch_page(self, page):
        assert self.paged
        self.bus.write_reg(self.addr, 0, bytes([page]), regaddrlen=self.regimmbytes)
        self.curr_page = page
    ...
    def write(self, reg, val, width=8):
        ...
        self.bus.write_reg(self.addr, imm, valbytes, regaddrlen=self.regimmbytes)
    ...
    def read(self, reg, width=8):
        ...
        data = self.bus.read_reg(
            self.addr, imm, width // 8, regaddrlen=self.regimmbytes
        )

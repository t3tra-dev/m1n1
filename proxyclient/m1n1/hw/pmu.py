    ...
    def reset_panic_counter(self):
        if self.primary and self.bus_type == "spmi":
            ...
            if self.node.compatible[0] in [
                ...
                counter = 0x5002
            elif self.node.compatible[0] in [
                "pmu,d2045",
                "pmu,d2089",
                "pmu,d2186",
                "pmu,d2207",
    ...
    def find_primary_pmu(adt):
        for child in adt["/arm-io"]:
            if child.name.startswith("nub-spmi") or child.name.startswith("spmi"):
                for pmu in child:
                    compat = (
                        getattr(pmu, "compatible")[0]
                        if hasattr(pmu, "compatible")
                        else "unset"
                    )
                    primary = (
                        (getattr(pmu, "is-primary") == 1)
                        if hasattr(pmu, "is-primary")
                        else False
                    )

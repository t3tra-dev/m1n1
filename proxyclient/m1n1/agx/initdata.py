...
def build_iomappings(agx, chip_id):
    ...
    if chip_id == 0x8103:
        ...
        return [
            ...
            iomap(0x204D61000, 0x1000, 0x1000, 1),  # GMGIFAFRegs
            iomap(
                0x200000000, mcc_cnt[chip_id] * 0xD8000, 0xD8000, 1
            ),  # MCache registers
            IOMapping(),  # AICBankedRegisters
            IOMapping(),  # PMGRScratch
            iomap(
                0x2643C4000, 0x1000, 0x1000, 1
            ),  # NIA Special agent idle register die 0
            (
                iomap(0x22643C4000, 0x1000, 0x1000, 1)
                if chip_id == 0x6002
                else IOMapping()
...
CHIP_INFO = {
    0x8103: Container(
        ...
        shared1_tab=[-1, 0x7282, 0x50EA, 0x370A, 0x25BE, 0x1C1F, 0x16FB] + ([-1] * 9),
        shared1_a4=0xFFFF,
    ...
    0x6001: Container(
        ...
        unk_3d34_0=[0, 0, 0, 0, 0, 0, 0, 0],
        unk_118ec=[0, 1, 2, 1, 1, 90, 75, 1, 1, 1, 2, 90, 75, 1, 1, 1, 1, 90, 75, 1, 1],
        hwdb_4e0=4,
        hwdb_534=1,
        num_cores=32,
        gpu_core=13,
    ...
    0x6002: Container(
        ...
        shared2_tab=[-1, -1, -1, -1, 0x2AA, 0xAAA, -1, -1, 0, 0],
        shared2_unk_508=0xCC00001,
        unk_3cf4=[1244.0, 1260.0, 1242.0, 1214.0, 1072.0, 1066.0, 1044.0, 1042.0],
...
def build_initdata(agx):
    ...
    for i in range(count):
        ...
        hwdata.rel_boost_freqs[i] = max(
            0, int((ps.freq - base_freq) / (max_freq - base_freq) * 100)
        )

    cs_pstates = sgx.getprop("cs-perf-states", None)
    if cs_pstates:
        hwdata.cs_max_pstate = cs_pstates.count - 1
        hwdata.cs_frequencies = [
            (ps.freq // 1000000) for ps in cs_pstates.states[0]
        ] + [0] * (16 - cs_pstates.count)
        hwdata.cs_voltages = [[(ps.volt // 1000), 0] for ps in cs_pstates.states[0]] + [
            [0, 0]
        ] * (16 - cs_pstates.count)
        hwdata.cs_voltages_sram = [
            [max(i[0], cs_pstates.min_sram_volt[0] // 1000) if i[0] else 0, 0]
            for i in hwdata.cs_voltages
        ]
    else:
    ...
    if afr_pstates:
        ...
        hwdata.afr_frequencies = [
            (ps.freq // 1000000) for ps in afr_pstates.states[0]
        ] + [0] * (8 - afr_pstates.count)
        hwdata.afr_voltages = [
            [(ps.volt // 1000), 0] for ps in afr_pstates.states[0]
        ] + [[0, 0]] * (8 - afr_pstates.count)
        hwdata.afr_voltages_sram = [
            [max(i[0], afr_pstates.min_sram_volt[0] // 1000) if i[0] else 0, 0]
            for i in hwdata.afr_voltages
        ]
    else:
    ...
    agx.uat.iomap_at(
        0,
        0x420000000,
        regionB.buffer_mgr_ctl._paddr_align,
        0x4000,
        AttrIndex=MemoryAttr.Shared,
        AP=0,
        UXN=1,
    ...
    regionB.buffer_mgr_ctl_gpu_addr = 0x420000000 + (
        regionB.buffer_mgr_ctl._addr & 0x3FFF
    )
    ...
    initdata.regionC = agx.kshared.new(
        InitData_RegionC(sgx, chip_info), track=False
    ).push()

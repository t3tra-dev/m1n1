...
class GPURenderer:
    def __init__(self, ctx, buffers=16, bm_slot=0, queue=0):
        ...
        self.job_list.last_head = (
            self.job_list._addr
        )  # Empty list has self as last_head
    ...
    def submit(self, cmdbuf, wait_for=None):
        ...
        unk_buf = self.ctx.uobj.new(
            Array(0x800, Int64ul), "Unknown Buffer", track=False
        )
        work.add(unk_buf)
        ...
        rgn_entry_size = 5
        tiling_params.size1 = (
            rgn_entry_size * tiles_per_mtile_x * tiles_per_mtile_y + 3
        ) // 4
        # PPP_MULTISAMPLECTL
        ...
        if cmdbuf.depth_buffer:
            size = (
                align_pot(max(width, tile_width))
                * align_pot(max(height, tile_width))
                // 32
            )
        ...
        tvb_tilemap = ctx.uobj.new_buf(
            tvb_tilemap_size, "TVB Tilemap", track=False
        ).push()
        ...
        tvb_heapmeta = ctx.uobj.new_buf(
            tvb_heapmeta_size, "TVB Heap Meta", track=False
        ).push()
        work.add(tvb_heapmeta)

        unk_tile_buf1 = self.ctx.uobj.new_buf(
            tvb_tilemap_size * nclusters, "Unk tile buf 1", track=False
        )
        print("tvb_tilemap_size", hex(tvb_tilemap_size))
        unk_tile_buf2 = self.ctx.uobj.new_buf(
            0x4 * nclusters, "Unk tile buf 2", track=False
        )
        # size = 0xc0 * nclusters
        size = 0xC80
        ...
        f4 = 0x280
        # f4 = 0x400 # t602x?
        unk_tile_buf4 = self.ctx.uobj.new_buf(
            f4 * nclusters, "Unk tile buf 4", track=False
        )
        f5 = 0x30
        # f5 = 0x980
        unk_tile_buf5 = self.ctx.uobj.new_buf(
            f5 * nclusters, "Unk tile buf 5", track=False
        )
        ...
        wc_3d.ts_pointers = TimeStampPointers(
            work.ts3d_start._addr, work.ts3d_end._addr
        )
        wc_3d.user_ts_pointers = TimeStampPointers(
            work.ns3d_start._addr, work.ns3d_end._addr
        )
        ...
        for i in cmdbuf.attachments[: cmdbuf.attachment_count]:
            ...
            order = 1  # FIXME
            start_3d.attachments.append(
                Attachment(i.pointer, cache_lines, 0x17, order)
            )  # FIXME check
            if work.fb is None and i.type == ASAHI_ATTACHMENT_C:
                work.fb = i.pointer
            if work.depth is None and i.type == ASAHI_ATTACHMENT_Z:
                work.depth = i.pointer
        start_3d.attachments += [Attachment(0, 0, 0, 0)] * (
            16 - len(start_3d.attachments)
        )
        ...
        wc_ta.ts_pointers = TimeStampPointers(
            work.tsta_start._addr, work.tsta_end._addr
        )
        wc_ta.user_ts_pointers = TimeStampPointers(
            work.nsta_start._addr, work.nsta_end._addr
        )
        ...
        if not use_registers:
            ...
            wc_ta.struct_2.tvb_heapmeta_addr = (
                tvb_heapmeta._addr | 0x8000_0000_0000_0000
            )
            ...
            wc_ta.struct_2.iogpu_deflake_3 = (
                deflake_3._addr | 0x4_0000_0000_0000
            )  # check
            ...
            wc_ta.struct_2.tvb_cluster_meta4 = (
                unk_tile_buf5._addr | 0x3000_0000_0000_0000
            )
            ...
            regs = {
                ...
                0x10129: tiling_params.x_max
                | (tiling_params.y_max << 16),  # PPP_SCREEN
    ...
    def wait(self):
        ...
        while True:
            ...
            if (
                self.stamp_3d1.value >= self.stamp_value_3d
                and self.stamp_ta1.value >= self.stamp_value_ta
            ):
                break
        ...
        if work.fb is not None and work.width and work.height and work.width == 1920:
            ...
            self.agx.p.fb_blit(
                0, 0, work.width, work.height, obj._paddr, work.width, PIX_FMT.XBGR
            )
        ...
        if False:  # work.depth is not None:
            ...
            os.system(
                f"convert -size {work.width}x{work.height} -depth 8 rgba:depth.bin -alpha off depth.png"
            )

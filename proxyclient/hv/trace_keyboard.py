...
class SIOEp(EP):
        ...
        if msg.EP == 0 and msg.PARAM == 0x1:
            ...
            if self.state.iova is not None and self.tracer.verbose > 1:
                iomon.add(
                    self.state.iova,
                    msg.DATA * 8,
                    name=f"sio.shmem@{self.state.iova:08x}",
            ...
            if self.state.iova is not None and self.tracer.verbose > 1:
                iomon.add(
                    self.state.iova_cfg,
                    msg.DATA * 8,
                    name=f"sio.shmem@{self.state.iova_cfg:08x}",
    ...
    def log_spihid_pkt(self, label, data):
        ...
        if pkt.crc16 != crc16:
            self.log(
                f"{label}: CRC mismatch: pkt.crc16:{pkt.crc16:#04x} crc16:{crc16:#04x}"
            )
            chexdump(data)
            return
        self.log(
            f"{label}: flags:{pkt.flags:#2x} dev:{pkt.dev:#2x} length:{pkt.length:4d} offset:{pkt.offset:3d} remain:{pkt.remain:3d}"
        )

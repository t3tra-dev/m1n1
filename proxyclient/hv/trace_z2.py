...
from m1n1.trace.asc import DIR, EP, ASCRegs, ASCTracer, EPState, msg, msg_log
...
class Z2Tracer(SPITracer):
    ...
    def main_z2(self):
        if self.txbuffer[0] == 0xEE:
            ...
            self.log(
                f"got report info for {self.txbuffer[1]}, len is {self.rxbuffer[3]}"
            )
            self.consume_bytes(16)
    ...
    def read_interrupt_data(self):
        ...
        tx = [
            f"TS1 {hex(data[1])} TS2 {tstamp2} UNK1: {mxformat(data[7:16])} UNK2: {mxformat(data[17:24])}"
        ]
        if len(data) >= 16:
            ntouch = data[16]
            for i in range(ntouch):
                ...
                tx.append(
                    f"F: {hex(finger)} S: {hex(state)} X: {x} Y: {y} MAJ: {wj} MIN: {wn} ANG: {dg} PRS: {prs} UNK1: {mxformat(data[ptr + 2:ptr+4])} UNK2: {mxformat(data[ptr + 8:ptr+12])} UNK3: {mxformat(data[ptr + 20:ptr+30])}"
                )
            self.log(";".join(tx))
        else:
...
spi_pins = {0x6D: "enable_cs", 0x8B: "reset"}

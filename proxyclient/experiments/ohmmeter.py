...
def init_ring_tip_sense():
    ...
    l84.regs.RING_SENSE_CTRL.set(
        INV=1,
        UNK1=1,
        RISETIME=E_DEBOUNCE_TIME.T_125MS,
        FALLTIME=E_DEBOUNCE_TIME.T_125MS,
    )
    l84.regs.TIP_SENSE_CTRL.set(
        INV=1, RISETIME=E_DEBOUNCE_TIME.T_500MS, FALLTIME=E_DEBOUNCE_TIME.T_125MS
    )
...
while True:
    print("Waiting for plug... ", end="")
    sys.stdout.flush()
    wait_for_plug()

    print("measuring... ", end="")
    sys.stdout.flush()
    print(f"{sense_Z():.1f} ohms... ", end="")
    sys.stdout.flush()

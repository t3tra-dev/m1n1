...
for i in range(1, 10):
    u.msr(
        DAIF, 0x140, call=lambda x, *args: p.smp_call_sync(i, x | REGION_RX_EL1, *args)
    )
    u.msr(
        DAIF, 0x1C0, call=lambda x, *args: p.smp_call_sync(i, x | REGION_RX_EL1, *args)
    )
    u.msr(
        (3, 4, 15, 10, 4),
        0,
        call=lambda x, *args: p.smp_call_sync(i, x | REGION_RX_EL1, *args),
    )
    mpidr = u.mrs(
        MIDR_EL1,
        call=lambda x, *args: p.smp_call_sync(i, x | REGION_RX_EL1, args[0], args[1]),
    )
    print(i, hex(mpidr))

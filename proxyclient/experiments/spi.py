...
for i in range(2):
    ...
    while regs.TXCNT.val != 0:
        print(
            f"{regs.TXCNT.val:#x} {regs.FIFOSTAT.reg} {regs.STATUS.val:#x} {regs.IF_FIFO.val:#x} {p.read32(spi + 0x134):#x}"
        )
        regs.STATUS.val = 0xFFFFFFFF
        regs.IF_XFER.val = 0xFFFFFFFF
        regs.IF_FIFO.val = 0xFFFFFFFF
    ...
    time.sleep(0.001)
    print(
        f"{regs.RXCNT.val:#x} {regs.FIFOSTAT.reg} {regs.STATUS.val:#x} {regs.IF_FIFO.val:#x}"
    )
    regs.STATUS.val = 0xFFFFFFFF
    regs.IF_XFER.val = 0xFFFFFFFF
    regs.IF_FIFO.val = 0xFFFFFFFF

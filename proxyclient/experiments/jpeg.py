            ...
            if seg_marker == 0xFFC0:
                ...
                if jpeg_components_cnt == 1:
                    ...
                    for i in range(jpeg_components_cnt):
                        comp_id, comp_sampling, _ = seg_data[
                            6 + 3 * i : 6 + 3 * (i + 1)
                        ]
...
if args.decode:
    ...
    with Image.new(
        mode="RGBA", size=(jpeg_W // decode_scale, jpeg_H // decode_scale)
    ) as im:
        if pixfmt in ["RGBA", "BGRA", "RGB565"]:
            ...
            for y in range(jpeg_H // decode_scale):
                for x in range(jpeg_W // decode_scale):
                    ...
                    cr = output_data[
                        surface_P1_off + y * surface_P1_stride + (x & ~1) + 1
                    ]
            ...
            for y in range(jpeg_H // decode_scale):
                for x in range(jpeg_W // decode_scale):
                    y_ = output_data[y * surface_stride + x]
                    cb = output_data[
                        surface_P1_off + (y // 2) * surface_P1_stride + x & ~1
                    ]
                    cr = output_data[
                        surface_P1_off + (y // 2) * surface_P1_stride + (x & ~1) + 1
                    ]

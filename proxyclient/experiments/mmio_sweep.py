...
argparser.add_argument(
    ...
    "--domain",
    type=str,
    help="Look for MMIO range associated with a particular" " power domain",
)
argparser.add_argument(
    "-p", "--print", action="store_true", help="Print power domain list"
)
...
for dev in pmgr.devices:
    ...
    ps_deps[dev.name] = [
        ps_dev_by_id[idx].name
        for idx in u.adt.pmgr_dev_get_parents(dev)
        if idx in ps_dev_by_id
    ]
...
def ps_set_pstate(name, desired):
    ...
    if actual & 0xF0 != desired << 4:
        print(
            "WARNING: %s stuck at pstate 0x%x (desired 0x%x)"
...
if args.domain:
    ...
    for dev in reversed(pmgr.devices):
        if dev.name not in to_enable or ps_enabled(dev.name):
            continue

...
if __name__ == "__main__":
    ...
    #
    parser.add_argument(
        "-i", "--input", type=str, required=True, help="path to input bitstream"
    )
    ...
    parser.add_argument(
        "--save-raw", type=str, default="", help="file name to save raw yuv"
    )
    parser.add_argument(
        "--save-images", type=str, default="", help="dirname to save images"
    )
    ...
    for i, unit in enumerate(units[:num]):
        ...
        if frame != None:
            ...
            if args.save_images:
                ...
                path = os.path.join(
                    f"data/out/{args.save_images}", "out%03d.png" % (avd.decoder.count)
                )
